...
class AnnotationService {
  ...
  saveAnnotation(annotation) {
    if (annotation != null) {
      ...
      if (this.ConfigService.isPreview()) {
        ...
        let httpParams = {
          method: "POST",
          url: this.ConfigService.getConfigParam(url),
  ...
  /**
   * Create an inappropriate flag annotation
   * @param runId the run id
   * @param periodId the period id
   * @param nodeId the node id
   * @param componentId the component id
   ...
   * @returns the inappropriate flag annotation
   */
  createVoteAnnotation(runId, periodId, nodeId, componentId,
    fromWorkgroupId, toWorkgroupId, studentWorkId, data) {
  const annotationId = null;
  const localNotebookItemId = null;
  const notebookItemId = null;
  ...
  getLatestVoteAnnotationByStudentWorkIdAndFromWorkgroupId(studentWorkId, fromWorkgroupId) {
    for (let a = this.annotations.length - 1; a >= 0; a--) {
      ...
      if (annotation != null) {
        if ("vote" == annotation.type && studentWorkId == annotation.studentWorkId && fromWorkgroupId == annotation.fromWorkgroupId) {
          /*
